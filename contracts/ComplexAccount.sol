 ...
 */
contract ComplexAccount is BaseAccount {
    ...
    function getAllGaurdians() public view returns (address[] memory) {
        return guardians;
    }
    ...
    function _onlyOwner() internal view {
        ...
        require(
            isSigner[msg.sender] || msg.sender == address(this),
            "only owner"
        );
    }
    ...
    function _validateSignature(
        ...
        require(
            isSigner[signer] || tx.origin == address(0),
    ...
    function recoverWallet(
        ...
        require(
            _signatures.length == guardians.length,
            "Invalid signatures length"
        );
        bytes32 hash = keccak256(
            abi.encodePacked(address(this), _newSigner, _nonce++)
        ).toEthSignedMessageHash();

        for (uint i = 0; i < _signatures.length; i++) {
            require(
                hash.recover(_signatures[i]) == guardians[i],
                "Invalid signatures"
            );
        }
    ...
    function gethash(address _newSigner) public view returns (bytes32 hash) {
        hash = keccak256(abi.encodePacked(address(this), _newSigner, _nonce));
        return hash;
    }
}
